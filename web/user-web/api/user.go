...
func PassWordLogin(c *gin.Context) {
	...
	if rsp, err := global.UserSrvClient.GetUserByMobile(context.WithValue(context.Background(), "ginContext", c), &proto.MobileRequest{
		...
		//只是查询到用户了而已，并没有检查密码
		if passRsp, pasErr := global.UserSrvClient.CheckPassWord(context.WithValue(context.Background(), "ginContext", c), &proto.PasswordCheckInfo{
			...
			if passRsp.Success {
				...
				c.JSON(http.StatusOK, gin.H{
					...
					"token":      token,
					"expired_at": (time.Now().Unix() + 60*60*24*30) * 1000, //浏览器jwt过期时间
				})
			} else {
				c.JSON(http.StatusBadRequest, map[string]string{
					"msg": "登录失败",
...
func GetUserDetail(ctx *gin.Context) {
	...
	rsp, err := global.UserSrvClient.GetUserById(context.WithValue(context.Background(), "ginContext", ctx), &proto.IdRequest{
...
func UpdateUser(ctx *gin.Context) {
	...
	_, err := global.UserSrvClient.UpdateUser(context.WithValue(context.Background(), "ginContext", ctx), &proto.UpdateUserInfo{
		Id:       int32(currentUser.ID),
