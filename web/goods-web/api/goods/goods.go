...
func List(ctx *gin.Context) {
	...
	eb, bb := sentinel.Entry("good_list_breaker")
	defer eb.Exit()
	if bb != nil {
		...
		//限流操作
		e, b := sentinel.Entry("goods_list", sentinel.WithTrafficType(base.Inbound))
		if b != nil {
...
func Detail(ctx *gin.Context) {
	//在大型系统里面 详情页包含的东西太多了 一般不会一次性全部加载(网络带宽有限 会卡) 采用异步加载方式加载(前端可以发多次请求)
	ids := ctx.Param("id")
	...
	if bb != nil {
		...
		e, b := sentinel.Entry("goods_detail", sentinel.WithTrafficType(base.Inbound))
		if b != nil {
			ctx.JSON(http.StatusTooManyRequests, gin.H{
				"msg": "请求过于频繁，请稍后重试",
			})
		...
		e.Exit()
		goodsInfoJson, err := goodsCache.Bytes()
		if err != nil {
...
func Delete(ctx *gin.Context) {
	...
	_, err = global.GoodsSrvClient.DeleteGoods(context.WithValue(context.Background(), "ginContext", ctx), &proto.DeleteGoodsInfo{Id: int32(id)})
	if err != nil {
		HandleGrpcErrorToHttp(err, ctx)
		return
	}
...
func Stocks(ctx *gin.Context) {
	...
	invInfo, err := global.InvClient.InvDetail(context.WithValue(context.Background(), "ginContext", ctx), &proto.GoodsInvInfo{
		GoodsId: int32(id),
	})
	if err != nil {
		HandleGrpcErrorToHttp(err, ctx)
